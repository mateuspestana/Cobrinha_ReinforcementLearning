...
class SnakeGameAI:
    ...
    # Função pra aleatorizar a posição da comida em relação à cobrinha
    ...
    def play_step(self, action):
        ...
        if self.is_collision() or self.frame_iteration > 120*len(self.snake):
            game_over = True
            reward = -10 # Se bater, o reward é negativo
            return reward, game_over, self.score

        # 4. Insere nova comida na tela
        if self.head == self.food:
            self.score += 1
            reward = 10 # Aqui é pra comida contar como +10
            self._place_food()
        else:
            self.snake.pop()
        ...
        # 6. Retorna o fim do jogo (com o reward e o score)
        return reward, game_over, self.score
